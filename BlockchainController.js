...
class BlockchainController {
    ...
    getBlockByHeight() {
        ...
        self.app.get("/block/:height", async (req, res) => {
            if (req.params.height) {
                ...
                let block = await self.blockchain.getBlockByHeight(height);
                if (block) {
    ...
    submitStar() {
        ...
        let self = this;
        self.app.post("/submitstar", async (req, res) => {
            if (req.body.address && req.body.message && req.body.signature && req.body.star) {
                ...
                try {
                    let block = await self.blockchain.submitStar(address, message, signature, star);
                    if (block) {
                        return res.status(200).json(block);
                    } else {
                        return res.status(500).send("An error happened!");
    ...
    getStarsByOwner() {
        ...
        self.app.get("/blocks/:address", async (req, res) => {
            if (req.params.address) {
                const address = req.params.address;
                try {
                    let stars = await self.blockchain.getStarsByWalletAddress(address);
                    if (stars) {
